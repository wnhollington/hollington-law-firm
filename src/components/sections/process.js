    ...
    return (
        <section id="process" ref={ref}>
            <div className={`container my-16 text-center animated animatedFadeInUp ${inView ? 'fadeInUp' : null}`}>
                ...
                <div className="flex flex-col md:flex-row gap-8 bg-white shadow-lg p-8">
                    {/* Tabs */}
                    <ul className="flex flex-col text-left md:w-2/4 xl:w-1/4 max-h-48 overflow-y-auto md:max-h-full">
                        {timeline.map((event) => {
                            return (
                                <li className={activeTab === event.id ? "bg-primary  text-white text-sm sm:text-lg font-bold p-4 rounded-lg" : "hover:shadow-lg hover:bg-primary hover:text-white hover:rounded-lg text-sm sm:text-lg font-bold p-4 "} onClick={() => setActiveTab(event.id)}>{`${event.id + 1}. ${event.point}`}</li>                    
                            )
                        })}
                    </ul>
                    {/* Content */}
